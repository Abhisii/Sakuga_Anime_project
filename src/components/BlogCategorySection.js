...
const BlogCategorySection = ({ title, categories }) => {
      ...
      <div className="relative max-w-[1200px] mx-auto overflow-hidden">
        ...
        >
          {categories.map(({ name, description, image }) => (
            <div
              key={name}
              className="relative group min-w-[420px] h-[260px] bg-[#131313] hover:bg-[#1E1E1E] text-white rounded-xl shadow-md hover:shadow-lg cursor-pointer overflow-hidden transition-all"
            >
              <img
                src={image}
                alt={name}
              ...
              <div
                className="absolute bottom-0 left-0 w-full h-0 group-hover:h-[35%] bg-black/80 text-white flex flex-col items-center justify-center overflow-hidden px-4 py-0 group-hover:py-3 rounded-b-xl transition-all duration-300 ease-in-out"
              >
                <h3 className="text-xl font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  {name}
                </h3>
